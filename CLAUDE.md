...
- ✅ **Prod gap found & fixed**: `/taxonomy/hospitals` had been silently EMPTY in production since launch — the CSV lives outside the backend Docker build context, so it never existed in the image. Fixed via read-only volume mount in `docker-compose.yml`; redeployed and verified 2026-07-23 (public API serves all 2,134 entries, e.g. St. Hedwig-Krankenhaus id 1557, 65 Berlin hospitals). Unblocks the future API-backed picker (see `project-mgmt/ticket-hospital-directory-maintenance.md`).
