...
- On every foreground finalization pass, if <7 days remain on the JWT, the app
  calls `POST /auth/refresh` and re-signs-in with the fresh 90-day token —
  active users never see a login screen again (server caps the chain at 365
  days since last interactive login).
- `WeekFinalizationService` pre-checks token expiry and maps 401/403 to a
  distinct `auth_expired` state (`AUTH_EXPIRED_ERROR` in the queue's
  `lastError`); the CalendarHeader banner offers tap-to-re-login (soft
  `signOut()` — local data untouched, queued weeks send after re-login).

**Honest week-send states** (CalendarHeader + ReportsScreen):
- Copy follows reality: scheduled ("sends Sunday evening", current week only) →
  "Sending…" (gerund while attempting/retrying) → "Sent ✓" (past tense, backend
  confirmed) → failure warning after ~48h of retries.
- ReportsScreen runs `sendEligibleQueuedWeeks()` on every load/focus, so queued
  PAST weeks transmit immediately instead of waiting for an app relaunch.
- Known deferred refactor: these send sub-states are derived in the UI; they
  belong in `WeekStateService` if a third consumer ever appears.

**Hospital directory + "my hospital is missing" fallback** (`ProfileForm`, `ProfileScreen`):
- Picker data: bundled per-state JSONs in `src/lib/taxonomy/hospitals/`
  (regenerated by `scripts/convert-hospitals-csv.py` from the repo-root CSV;
  ~1,830 hospitals since the 2026-07 OSM rebuild). **Ids are stable and
  user-referenced — never regenerate in a way that changes existing ids**
  (the CSV has an explicit `id` column; append-only).
- Two pinned picker options with DIFFERENT semantics: `missing` (dataset gap →
  inline free-text input, name stored in legacy `hospital_id`, ref id NULL)
  and `other` (privacy opt-out, unchanged).
- The un-confirm flow (`useDayLock`) runs cross-store side effects BEFORE the
  state flip — a partial failure leaves the day confirmed (data-safe) rather
  than risking a stale-hours submission.
