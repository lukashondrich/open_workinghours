...
to determine optimal ε, K_MIN, split, dominance threshold, and temporal
...
See: project-mgmt/dp-group-stats-simulation-spec.md
"""
...
from app.dp_group_stats.config import ContributionBounds
...
def run_scenario(
    ...
    # If annual_cap is set, compute ε per period = cap / n_periods
...
def main() -> None:
    ...
    parser.add_argument("--annual-cap", type=float, default=None, help="Annual ε cap (overrides --epsilon, sets ε=cap/n_periods)")
    ...
    if args.sweep:
        run_spread_sweep(args)
    elif args.sweep_pilot:
        run_pilot_sweep(args)
...
def run_single(args: argparse.Namespace) -> None:
    ...
    print(
        f"Running: {mode}, users={args.users}, ε={args.epsilon}, "
...
def run_spread_sweep(args: argparse.Namespace) -> None:
...
def run_pilot_sweep(args: argparse.Namespace) -> None:
...
def run_dynamic_sweep(args: argparse.Namespace) -> None:
    ...
    results = []
    configs = [
        # (pilot, hospital_level, period, dynamic, label_suffix)
        (True, False, "monthly", False, "specialty"),
        (True, False, "biweekly", False, "specialty"),
        (True, False, "weekly", False, "specialty"),
        (True, False, "weekly", True, "specialty"),   # dynamic
        (True, True, "monthly", False, "hospital"),    # hospital-level
        (True, True, "weekly", True, "hospital"),      # hospital-level + dynamic
    ]
    ...
    for n_users in users_values:
        for pilot, hosp, period, dyn, _ in configs:
            r = run_scenario(
                n_users=n_users, epsilon=1.0,  # overridden by cap
